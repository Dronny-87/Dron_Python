...
def translate_sentence_glosbe_with_fallback(text, from_lang='sr', to_lang='ru'):
    ...
    for word in words:
        ...
        # Добавляем все варианты переводов
        translations.append(f"Для слова '{word}' варианты перевода: {', '.join(word_translations)}")
...
if __name__ == "__main__":
    while True:  # Бесконечный цикл для повторных запросов
        ...
        # Вывод результатов
        print("Возможные переводы от Glosbe_v2:")
        print(translations_glosbe)

        print("Возможные переводы от Google Translate:")
